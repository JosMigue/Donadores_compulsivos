        ...
        <table class="table table-hover table-striped table-sm text-center">
          ...
          <tbody>
            <tr v-for="(campaigndonor, index) in campaigndonors.data" :key="index">
              ...
              <td>
                <button v-if="campaigndonor.pivot.donor_donated" v-on:click="changeStatusDonation(index, 0)" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="right" title="Marcar como no asistió"><i class="fa fa-times"></i></button>
